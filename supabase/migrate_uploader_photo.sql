...
-- Jalankan di SQL Editor PROJECT FOTO.
-- uploader: boleh upload foto, TIDAK boleh hapus.
-- admin: boleh upload + hapus.
-- visitor: hanya melihat.
-- ============================================================

-- File ini melakukan hal yang sama dengan add_uploader_role.sql
-- tetapi namanya dibuat eksplisit agar tidak tertukar dengan project video.
...
to authenticated
with check (bucket_id='class-photos' and exists (select 1 from public.profiles where id=auth.uid() and role in ('admin','uploader')));
...
to authenticated
using (bucket_id='class-photos' and exists (select 1 from public.profiles where id=auth.uid() and role='admin'));
